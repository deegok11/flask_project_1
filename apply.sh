...
original_name="flask_project_1"
original_urlname="flask_project_1"
original_description="Awesome flask_project_1 created by deegok11" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_project_1 "${name}"
...
then
    ...
    echo "then run 'flask_project_1' or 'python -m flask_project_1'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_project_1' or 'python -m flask_project_1' or 'poetry run flask_project_1'"
fi
